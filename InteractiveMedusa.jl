...
function medusa_model(yaw,u,v,r,us,up)
    #nota, la dinámica es independiente de x e y por eso sólo tengo como
    #entrada yaw

    #%%%%%%%%%%%%%% Trusters %%%%%%%%%%%%%
    eta=1.77778e-6; #constante de conversión W-F
    u_max=100.0; 

    us=min(u_max,max(-u_max,us)); #saturation
    up=min(u_max,max(-u_max,up));

    rpm_s=45.0*us; #rev per minute
    rpm_p=45.0*up;

    Fs=eta * abs(rpm_s)* rpm_s;
    Fp=eta * abs(rpm_p)* rpm_p;
